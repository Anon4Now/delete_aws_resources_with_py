...
class Resources:
    ...
    def get_vpc_resources(self, selection: str):
        """
        Return the resource information for the passed in var, will accept any of the following:
        CREATE DOCUMENTATION HERE
        :param selection: A passed VPC resource that corresponds to a dict key
        :return:
        """
        if selection and isinstance(selection, str):
            item_selection = {
                "igw": self.current_vpc_resource.internet_gateways.all(),
                "subnet": self.current_vpc_resource.subnets.all(),
            }
            return item_selection[selection]
        else:
            logger.error("[-] Need to pass a string var that corresponds to one of the default vpc resources")
...
class AlterResources(Resources):
    ...
    def delete_resources(self):
        internet_gateways = self.get_vpc_resources("igw")
    ...
    def call_methods(self):
        ...
        for current_region in self.region_list:
            vpc_id = self.get_vpcs(current_region)
            self.current_vpc_resource = create_boto3_resource(self.resource, region=current_region).Vpc(vpc_id)
            if self.args == 'all':
